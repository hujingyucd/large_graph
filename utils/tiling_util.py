import numpy as np
import shapely
from utils.algo_util import align
...
def align_tile(base_seg_p0, base_seg_p1, tile_poly, edge_idx, align_mode):
    ...
    R, T = align(base_seg_p0, base_seg_p1, tile_edge_p0, tile_edge_p1,
                 align_mode)
...
def get_first_touch_point(tile_1, tile_2):
    ...
    for i in range(tile_1.get_edge_num()):
        for j in range(tile_2.get_edge_num()):
            ...
            # the segment is also colinear with the cross segment
            if abs(abs(normalize(line_a).dot(normalize(line_b))) -
                   1.0) < EPS and (abs(
                       abs(normalize(line_a).dot(normalize(b_1 - a_2))) -
                       1.0) < EPS or abs(
                           abs(normalize(line_a).dot(normalize(b_2 - a_2))) -
                           1.0) < EPS):
                ...
                current_overlap = max(
                    0.0,
                    min(unit_a_2_basea, max(unit_b_1_basea, unit_b_2_basea)) -
                    max(unit_a_1_basea, min(unit_b_1_basea, unit_b_2_basea)))
                if current_overlap > EPS:
                    point_a = max(
                        unit_a_1_basea,
                        min(unit_b_1_basea,
                            unit_b_2_basea))  # point on edge i of tile_1
                    point_b = max(
                        unit_b_1_baseb,
                        min(unit_a_1_baseb,
                            unit_a_2_baseb))  # point on edge j of tile_2
