...
optional_variables_names = [
    ...
    'align_edge_index', 'align_edge_features', 'predict', 'predict_order',
    'target_shape', 'predict_probs'
]


def write_brick_layout_data(save_path,
                            re_index,
                            node_features=None,
...
def write_tree_search_layout(save_path, temp_layout, node_re_index):
    ...
    dic = {"temp_layout": temp_layout, "node_re_index": node_re_index}
...
def load_tree_search_layout(load_path):
    ...
    assert ("temp_layout" in f.keys() and "node_re_index" in f.keys())
...
def recover_features_from_reindex(re_index, complete_graph):
    ...
    filtered_collided_edges = [
        edge for edge in complete_graph.colli_edges
        if edge[0] in tiles_super_set and edge[1] in tiles_super_set
    ]
    filtered_adj_edges = [
        edge for edge in complete_graph.adj_edges
        if edge[0] in tiles_super_set and edge[1] in tiles_super_set
    ]
...
def generate_brick_layout_data(graph: TileGraph, super_tiles: list,
    ...
    super_edge_features_adj = np.array(
        [graph.edges_features[edge[0]][edge[1]] for edge in adj_edges])
    if len(super_edge_features_adj) > 0:
        super_edge_features_adj[:,
                                1] = super_edge_features_adj[:,
                                                             1] / graph.max_align_length
    ...
    # edge index
    edge_index_collide = list(
        map(lambda e: (re_index[e[0]], re_index[e[1]]), collide_edges))
    edge_index_align = list(
        map(lambda e: (re_index[e[0]], re_index[e[1]]), adj_edges))
