...
def align(base_segment_p0, base_segment_p1, tile_segment_p0, tile_segment_p1,
    ...
    base_vec = np.array([
        base_segment_p1[0] - base_segment_p0[0],
        base_segment_p1[1] - base_segment_p0[1], 0
    ])
    align_vec = np.array([
        tile_segment_p1[0] - tile_segment_p0[0],
        tile_segment_p1[1] - tile_segment_p0[1], 0
    ])
    rot_axis = np.cross(base_vec, align_vec)
    cos = np.dot(align_vec, base_vec) / (np.linalg.norm(align_vec) *
                                         np.linalg.norm(base_vec))
    cos = np.clip(cos, -1, 1)
    theta = math.acos(cos)
    if rot_axis[2] > 0:
        theta = -theta
...
class Replay_Buffer:
    ...
    def sample(self, batch_size):
        ...
        actions, rewards, dones = np.array(actions), np.array(
            rewards), np.array(dones)

        # current state
        current_contour, current_segments_len, current_mask, current_triangles_info = list(
            zip(*states))
        current_max_len = max(current_segments_len)
        current_contour = np.array([
            np.concatenate(
                (item,
                 np.zeros((current_max_len - item.shape[0], item.shape[1]))))
            for item in current_contour
        ])
        current_mask = np.array([
            np.concatenate((item, np.zeros((current_max_len - item.shape[0]))))
            for item in current_mask
        ])
        current_segments_len = np.array(current_segments_len)
        current_triangles_info = np.array(current_triangles_info)
        states = (current_contour, current_segments_len, current_mask,
                  current_triangles_info)

        # next state
        next_contour, next_segments_len, next_mask, next_triangles_info = list(
            zip(*next_states))
        next_max_len = max(next_segments_len)
        next_contour = np.array([
            np.concatenate(
                (item, np.zeros(
                    (next_max_len - item.shape[0], item.shape[1]))))
            for item in next_contour
        ])
        next_mask = np.array([
            np.concatenate((item, np.zeros((next_max_len - item.shape[0]))))
            for item in next_mask
        ])

        # handle of termination
        next_mask[:, 0] = 1
        next_segments_len = np.clip(np.array(next_segments_len),
                                    a_min=1,
                                    a_max=None)

        next_triangles_info = np.array(next_triangles_info)
        next_states = (next_contour, next_segments_len, next_mask,
                       next_triangles_info)
...
def process_training_time(base_folder):
    ...
    for file_name in subfolders:
        ...
        names = [
            os.path.basename(x)
            for x in glob.glob(os.path.join(file_name, file_type))
        ]
        filtered_time = [
            float(name.split("_")[2][:-4]) for name in names
            if name.split("_")[0][:4] == "time"
        ]
