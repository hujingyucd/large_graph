...
import math
import pickle
import utils.tiling_util as tiling_util
...
def get_tile_instance(base_tile: Tile, base_edge_idx, align_tile: Tile,
                      align_edge_idx, align_mode):
    base_edge_p0, base_edge_p1 = base_tile.get_edge(base_edge_idx)
    tile_instance = tiling_util.align_tile(base_edge_p1, base_edge_p0,
                                           align_tile.tile_poly,
                                           align_edge_idx, align_mode)
    ...
    for base_edge_idx in range(base_tile.get_edge_num()):
        for align_neighbor_idx in range(align_tile.get_edge_num()):
            for align_mode in [0, 1]:
                align_tag = (base_tile.id, align_tile.id, base_edge_idx,
                             align_neighbor_idx, align_mode)
                if integer_align:
...
def find_candidate_tile_locations(num_rings,
                                  base_tile: Tile,
                                  align_tiles: list,
                                  integer_align=True):
    # the resulting tiles
    ...
    for i in range(0, num_rings):
        ...
        for last_ring_idx in range(last_ring_num):
            ...
            for align_tile in align_tiles:
                neighbour_tiles, _ = get_all_tiles(last_layer_tile,
                                                   align_tile,
...
class TileGraph:
    ...
    def __init__(self,
        ...
        if proto_tiles is not None:
            ...
            self.total_feature_dim = self.align_start_index + len(
    ...
    def edge_features_mapping(self, proto_tiles):
        ...
        for center_tile in proto_tiles:
            ...
            for align_tile in proto_tiles:
                ...
                one_ring_tiles, _ = get_all_tiles(center_tile,
                                                  align_tile,
                                                  integer_align=True)
                ...
                for neigbor_tile in one_ring_tiles:
                    colli_area = tiling_util.intersection_area(
                        center_tile, neigbor_tile)
                    align_length = tiling_util.polygon_align_length(
                        center_tile, neigbor_tile)
                    edge_feature_back = self.to_edge_feature(
                    ...
                    edge_feature = self.to_edge_feature_new(
                        colli_area, align_length, tile_i_align, tile_j_align,
                        center_tile.id, neigbor_tile.id)
                    reflected_feature = self.to_edge_feature_new(
                        colli_area, align_length, tile_j_align, tile_i_align,
                        neigbor_tile.id, center_tile.id)
                    ...
                    if edge_feature is not None:
                        ...
                        current_edge_feature = edge_feature[self.
                                                            align_start_index:]
                        current_reflected_feature = reflected_feature[
                            self.align_start_index:]
        ...
        for i, j in itertools.combinations(range(len(unique_features)), 2):
            assert ((not (unique_features[i][2] == unique_features[j][2]
                          and unique_features[i][3] == unique_features[j][3])
                     or abs(unique_features[i][0] - unique_features[j][0]) +
                     abs(unique_features[i][1] - unique_features[j][1]) > EPS)
                    or not (unique_features[i][2] == unique_features[j][3]
                            and unique_features[i][3] == unique_features[j][2])
                    or abs(unique_features[i][0] - unique_features[j][1]) +
                    abs(unique_features[i][1] - unique_features[j][0]) > EPS)
    ...
    def _form__graph(self):
        ...
        for i, j in itertools.combinations(range(len(self.tiles)), 2):
            colli_area = tiling_util.intersection_area(self.tiles[i],
                                                       self.tiles[j])
            align_length = tiling_util.polygon_align_length(
            ...
            # reflected feature
            edge_feature = self.to_edge_feature_new(colli_area, align_length,
                                                    tile_i_align, tile_j_align,
                                                    self.tiles[i].id,
                                                    self.tiles[j].id)
            reflected_feature = self.to_edge_feature_new(
                colli_area, align_length, tile_j_align, tile_i_align,
                self.tiles[j].id, self.tiles[i].id)
            # assertion of correctness
            assert edge_feature_back is None or \
                   edge_feature is None or \
                   (edge_feature[0] == edge_feature_back[0] and
    ...
    @staticmethod
    def to_edge_feature_new(colli_area, align_length, align_portion_1,
                            align_portion_2, tile_index_1, tile_index_2):
        if colli_area > 1e-6:  # contain intersection
            return [
                colli_area, 0, align_portion_1, align_portion_2, tile_index_1,
                tile_index_2
            ]
        elif align_length > 1e-6:
            return [
                0, align_length, align_portion_1, align_portion_2,
                tile_index_1, tile_index_2
            ]
        else:
            return None
    ...
    def feature_to_one_hot(self, input_feature):
        ...
        input_reflected_feature = [
            input_adj_feature[1], input_adj_feature[0], input_adj_feature[3],
            input_adj_feature[2]
        ]
    ...
    # def show_complete_super_graph(self, plotter, file_name):
