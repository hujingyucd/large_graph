...
import numpy as np
from utils.algo_util import _distance
...
class Tile:
    ...
    def __eq__(self, other):
        ...
        if self.tile_poly.almost_equals(other.tile_poly, decimal=5):
            ...
            return abs(
                self.tile_poly.intersection(other.tile_poly).area -
                self.tile_poly.area) < ESP
    ...
    def get_perimeter(self):
        return np.sum(
            [self.get_edge_length(i) for i in range(self.get_edge_num())])
