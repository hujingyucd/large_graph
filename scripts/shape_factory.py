...
def read_binary_image(file_path):
    ...
    # filtered_contour = []
    polygons_lists = [
        Polygon(np.array(contour).squeeze()) for contour in contours
    ]
    ...
    filtered_polygon_lists = list(
        filter(lambda contour_poly: contour_poly.area > filter_thershold,
               polygons_lists))
    ...
    interior_polygons_coords = [
        np.array(interior_polygon.exterior.coords)
        for interior_polygon in filtered_polygon_lists
    ]
...
def export_contour_as_text(output_path, contours):
    ...
    with open(output_path, 'w') as file:
        print(*['{0} {1}'.format(x, y) for x, y in exterior_polygon_coords],
              sep=',',
              file=file)
        for interior_polygon_coords in interior_polygons_coords:
            print(
                *['{0} {1}'.format(x, y) for x, y in interior_polygon_coords],
                sep=',',
                file=file)
...
def transform_all_binary_images(root_path):
    if os.path.isdir(root_path):
        ...
        if '.txt' not in root_path and not os.path.exists(target_contour_name):
            export_contour_as_text(target_contour_name,
                                   read_binary_image(root_path))
    else:
        raise FileNotFoundError('Invalid Filename')


if __name__ == '__main__':
    transform_all_binary_images(
        "/research/dept8/fyp21/cwf2101/rchuan/working/large_graph/data/silhouette_temp"
    )
